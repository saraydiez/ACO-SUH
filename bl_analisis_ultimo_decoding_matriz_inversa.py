# -*- coding: utf-8 -*-
"""BL ANALISIS ULTIMO Decoding_matriz_inversa.ipynb

Automatically generated by Colab.

Original file is located at
    https://colab.research.google.com/drive/18_e4liou3w3U4NAUSBQ3DyC02cp5Uiw3
...
def ejecuta_decoding(rep_solucion, df_pacientes, df_recursos_final, df_datos, array_pacientes):
...
FO_inicial_1, df_datos_unido, df_pacientes = ejecuta_decoding(rep_solucion_1, df_pacientes, df_recursos_final, df_datos,array_pacientes)
...
def BL_ejecuta_decoding(df_datos ,df_pacientes, solucion):
  # Diccionarios (clave: valor) para mayor legibilidad de los resultados
  tiempo_inicio_actividad={}
  tiempos_finalizacion_actividad={}
  consultas_asignadas = {}
...
def busqueda_local(solucion_inicial, df_datos, df_pacientes):
   # Copia de la solución inicial para no modificar el original
    ...
    for p in pacientes_sacados:
        ...
        for i in range(len(pacientes_mantenidos) + 1):
            ...
            FO_busqueda = BL_ejecuta_decoding(df_datos_mantenidos ,df_pacientes_mantenidos, new_solution)
            print(FO_busqueda)
...
def algoritmo_aco(num_hormigas, parametros ,Q_0, FO_inicial, solucion_inicial, actividades, orden_actividades_por_paciente, df_pacientes, df_recursos_final, df_datos, array_pacientes):
    ...
    while (time.time() - tiempo_inicial) < 81.0:
        iteracion += 1
        ...
        for hormiga in range(num_hormigas):
            ...
            FO_hormiga, df_datos_unido, df_pacientes = ejecuta_decoding(solucion_hormiga, df_pacientes, df_recursos_final, df_datos, array_pacientes)  # Implementar evaluación de la solución. Calcular su FO
...
# Algoritmo principal
def algoritmo_aco1(tiempo_computacion,array_pacientes, num_hormigas, parametros ,Q_0, FO_inicial, solucion_inicial, actividades, orden_actividades_por_paciente, df_pacientes, df_recursos_final, df_datos):
    # Inicializar parametros
    num_actividades = len(actividades)  # Ajustar según tu caso
    ...
    while (time.time() - tiempo_inicial) < tiempo_computacion:
        ...
        if iteracion >= 3 and len(set(best_hormiga_FO[-3:])) == 1:
          bl = 1
          best_sol_bl , best_FO_bl = busqueda_local(array_pacientes, df_datos, df_pacientes)
    ...
    best_sol = best_hormiga[best_hormiga_FO.index(min(best_hormiga_FO))]
    FO_best = min(best_hormiga_FO)
    return best_sol, df, FO_best, counter

# Algoritmo principal
def algoritmo_aco2(tiempo_computacion,array_pacientes, num_hormigas, parametros ,Q_0, FO_inicial, solucion_inicial, actividades, orden_actividades_por_paciente, df_pacientes, df_recursos_final, df_datos):
    # Inicializar parametros
    num_actividades = len(actividades)  # Ajustar según tu caso
...
# Algoritmo principal
def algoritmo_aco3(tiempo_computacion, array_pacientes, num_hormigas, parametros ,Q_0, FO_inicial, solucion_inicial, actividades, orden_actividades_por_paciente, df_pacientes, df_recursos_final, df_datos):
    # Inicializar parametros
    num_actividades = len(actividades)  # Ajustar según tu caso
...
# PARÁMETROS:
# Q01- P1= 0.6	P2=0.6	P3=0.4	P4=0.6	P5=	 P6=0.4
# Q02- P1= 0.4	P2=0.9	P3=0.4	P4=0.9	P5=	 P6=0.4
# Q03- P1= 0.4	P2=0.4	P3=0.4	P4=0.4	P5=	 P6=0.4
# N_HORMIGAS - P1 = P2 = 100 , P3 = 300 , P4=P5=P6= 50
import numpy as np
import random
import matplotlib.pyplot as plt
...
results = pd.DataFrame(columns=['ACO','BEST_SOL'])

best_sol1 , df1 , FO_ACO1, counter1 = algoritmo_aco1(tiempo_computacion,array_pacientes,num_hormigas, parametros, q01, FO_inicial_1, rep_solucion_1, actividades, orden_actividades_por_paciente, df_pacientes, df_recursos_final, df_datos)
...
plt.plot(df1['Iteracion'], df1['FO'], marker='o')
...
best_sol2 , df2 , FO_ACO2, counter2 = algoritmo_aco2(tiempo_computacion,array_pacientes, num_hormigas, parametros, q02, FO_inicial_1, rep_solucion_1, actividades, orden_actividades_por_paciente, df_pacientes, df_recursos_final, df_datos)
#FO_ACO1, df_datos_unido, df_pacientes = ejecuta_decoding(best_sol,array_prioridades , df_pacientes, df_recursos_final, df_datos)

plt.figure(figsize=(10, 6))
plt.plot(df2['Iteracion'], df2['FO'], marker='o')
plt.title('Mejores soluciones encontradas por iteración')
plt.xlabel('Iteración')
plt.ylabel('FO')
plt.legend()
...
best_sol3 , df3, FO_ACO3, counter3 = algoritmo_aco3(tiempo_computacion,array_pacientes,num_hormigas, parametros, q03, FO_inicial_1, rep_solucion_1, actividades, orden_actividades_por_paciente, df_pacientes, df_recursos_final, df_datos)
#FO_ACO1, df_datos_unido, df_pacientes = ejecuta_decoding(best_sol,array_prioridades , df_pacientes, df_recursos_final, df_datos)

plt.figure(figsize=(10, 6))
plt.plot(df3['Iteracion'], df3['FO'], marker='o')
plt.title('Mejores soluciones encontradas por iteración')
plt.xlabel('Iteración')
plt.ylabel('FO')
plt.legend()
...
# Algoritmo principal
def algoritmo_aco_pacientes(tiempo_computacion, array_pacientes, FO_inicial,solucion_inicial, num_hormigas, df_pacientes ,df_datos, alpha, beta, ro,q0):
    semilla = 0
    ...
    while (time.time() - tiempo_inicial) < tiempo_computacion:
        ...
        for n in range(num_hormigas):
            if bl == 0:
              ...
              FO_hormiga, df_datos_unido, df_pacientes = ejecuta_decoding(rep_solucion, df_pacientes, df_recursos_final, df_datos,array_pacientes)  # Implementar evaluación de la solución. Calcular su FO
        ...
        if iteracion >= 3 and len(set(mejor_solucion_FO[-3:])) == 1:
          bl = 1
          best_sol_bl , best_FO_bl = busqueda_local(solucion_inicial, df_datos, df_pacientes)
          print('nueva solucion', best_sol_bl)
          #best_calidad_bl = calcular_calidad_solucion(best_FO_bl)
          #actualizar_feromonas(matriz_feromonas, best_sol_bl, best_calidad_bl,ro)
    ...
    return mejor_solucion_global, df, mejor_FO, counter
...
num_hormigas = 100
tiempo_computacion = (((len(df_datos)*15) /2)*375)/1000 # a las 12h
best_sol4, df4, FO_ACO4, counter4 = algoritmo_aco_pacientes(tiempo_computacion,array_pacientes, FO_inicial_1,solucion_inicial, num_hormigas, df_pacientes ,df_datos, alpha, beta, tasa_evaporacion,q04)

plt.figure(figsize=(10, 6))
plt.plot(df4['Iteracion'], df4['FO'], marker='o')
plt.title('Mejores soluciones encontradas por iteración')
plt.xlabel('Iteración')
plt.ylabel('FO')
plt.legend()
...
# Calcular el mejor FO encontrado hasta cada iteración
combined_df['Best_FO'] = combined_df.groupby('Algoritmo')['FO'].cummin()

# Calcular métricas de estanqueidad
metrics = []


for algorithm in combined_df['Algoritmo'].unique():
    df_algo = combined_df[combined_df['Algoritmo'] == algorithm]

    # Número de Iteraciones para Mejor Solución (NIMS)
    best_iteration = df_algo[df_algo['FO'] == df_algo['Best_FO'].min()]['Iteracion'].min()

    # Tasa de Convergencia: diferencia promedio en la calidad de la solución por iteración
    convergence_rate = df_algo['Best_FO'].diff().mean()

    # Duración del Estancamiento: número de iteraciones sin mejora
    stagnation_duration = (df_algo['Best_FO'].diff() == 0).sum()

    metrics.append({
        'Algoritmo': algorithm,
        'NIMS': best_iteration,
        'Tasa de Convergencia': convergence_rate,
        'Duración del Estancamiento': stagnation_duration
    })
...
print('Mejores soluciones encontradas por','\nACO1:', counter1,'\nACO2:', counter2,'\nACO3:', counter3,'\nACO4:', counter4)
